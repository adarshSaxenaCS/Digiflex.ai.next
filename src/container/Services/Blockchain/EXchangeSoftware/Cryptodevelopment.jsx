...
import { FaExchangeAlt, FaWallet, FaLayerGroup, FaStore, FaCubes } from "react-icons/fa";
...
const Cryptodevelopment = () => {
  ...
  const isInView = useInView(sectionRef, { once: true });  // ✅ Corrected

  return (
    <WrapperContainer>
      <div>
        <Heading>Our Crypto Exchange Development Solutions</Heading>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 xl:grid-cols-3 gap-6 max-w-6xl mx-auto text-center">
            {cards.map((card, index) => (
              ...
              >
                ...
                <motion.div
                  className="absolute bottom-0 left-0 w-full h-2/5 bg-gray-100 flex items-center justify-center text-gray-800 text-xs sm:text-sm font-medium p-2 sm:p-3 rounded-b-xl border-t border-gray-300"
                  initial={{ bottom: "-100%" }}
                  whileHover={{ bottom: "0%" }}
